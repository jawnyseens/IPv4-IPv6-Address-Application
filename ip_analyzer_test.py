import unittest
# Assuming your main app code is in 'ip_analyzer.py'
# NOTE: Ensure your main file is named 'ip_analyzer.py'
from ip_analyzer import validate_ip, analyze_consistency, privacy_exposure_score

# Mock API results used for testing analysis functions
# MOCK_RESULT_1 and MOCK_RESULT_2_SAME share country 'US' and timezone 'America/Los_Angeles' (IPv4)
MOCK_RESULT_1 = {
...
# MOCK_RESULT_3_DIFFERENT has country 'JP' and timezone 'Asia/Tokyo' (IPv6)
...
class TestAnalyzerFunctions(unittest.TestCase):
    ...
    def test_consistency_high(self):
        ...
        # Expect high consistency (3/3 same country)
    ...
    def test_consistency_low(self):
        ...
        results = [MOCK_RESULT_1, MOCK_RESULT_3_DIFFERENT,
                   MOCK_RESULT_3_DIFFERENT]
        summary, score = analyze_consistency(results)
        # Expect 66.67% consistency (2/3 same country)
        self.assertAlmostEqual(score, (2/3) * 100, places=2)
    ...
    def test_privacy_score_ipv6(self):
        ...
        # CORRECTED EXPECTED SCORE:
        # Base (100)
        # - Mismatched Countries (-20, because US and JP are present)
        # - IPv6 Detected (-10)
        # - Timezone Mismatch (-15)
        # = 55
        self.assertEqual(score, 55)
        # Verify the 20-point penalty note is present
        self.assertIn("Inconsistent geolocation across APIs", notes[0])
        self.assertIn("IPv6 detected", notes[1])
        self.assertIn("Timezone inconsistency", notes[2])
